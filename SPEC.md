...
**§2–§9 现已全部落地。** 这四条曾长期是「规范而非现状」，保留在此是为了记录它们何时补齐——以及提醒：**这张表以后如果又出现 ❌，说明规范和代码再次脱节，应当立刻处理，而不是让它长期挂着**。
...
- **外部 dead-man switch 需自行配置**：服务端自检已闭环（心跳携带签名 `health`），B 层反向打点看门狗也已实现（`ALERTHUB_WATCHDOG_URL`），但它**只在使用者真的在第三方建了 check 之后才生效**——未配置时启动会 WARN，且服务器整体宕机将无人告警。
