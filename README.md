...
| `ALERTHUB_PUBKEYS_EXTRA` | （空）| 轮换重叠期额外**接受**的公钥（base64url raw，逗号分隔）。只验签、不签名。见 SPEC §8 |
...
| **P0** | fail-loud 心跳 + ntfy 独立通道 | ✅ **代码完成**：A 层签名心跳（10s）+ 浏览器看门狗（30s degraded / 60s offline）+ 时钟漂移横幅 + ntfy 双通道；服务端自检闭环（心跳带签名 `health`）；**B 层外部 dead-man switch**（健康打点 / 降级打 `/fail` / 死亡静默）。⚠️ 需配置 `ALERTHUB_WATCHDOG_URL` 才真正生效，未配置启动会 WARN |
