...
> 状态：🟡 **已实现**（模板与一键发送）。五个场景由**服务端**持有（`GET /api/scenarios`）——不是面板硬编码，因为各端必须提供**同一套措辞**：应急时人靠肌肉记忆,壁挂屏和手机上写着不同话的按钮比没有按钮更糟。`POST /api/publish/scenario {id, note?}` 一键发送；调用方可**追加**上下文,但**改不了指令本身**（`action` 由服务端固定）。每个场景映射到 CAP `responseType`（Evacuate/Shelter/AllClear）。`警报解除` 刻意是 `notice` —— 解除通知本身若也全屏接管,会让人连松一口气都变成惊吓。
> **仍缺**：目标组与通道集（依赖尚不存在的设备分组模型，见 RFC 0001）；模板尚不可由管理员自定义。
...
| P1 | CAP 1.2 ingest API + 出向 CAP；场景模板 | 🟡 部分完成 | 已做：CAP 1.2 解析、三元（severity/urgency/certainty）折叠、`Exercise/Test/System` 强制降为演练、`responseType→action`、**Cancel**（确定性 id + `<references>` → `CancelByID`，无映射表）。**`msgType=Update` 的 supersede 已实现**（Update 沿用 `<references>` 所指警报的 id 重发 → 客户端就地更新；未升级则静默，升级则重弹，见 SPEC §5.2）。**出向 CAP 已实现**：`GET /api/cap/alert` 渲染单条、`/api/cap/feed` 是内嵌 CAP 文档的 Atom feed；severity 反推的三元组经往返测试守护（保证折叠回同一 severity），演练以 `status=Exercise` 发出、绝不标 `Actual`。缺：`<area>`/geocode（JIS 市町村码）定向——**当前所有 CAP 警报都是全员**，且依赖尚不存在的设备位置模型（见 RFC 0001）；§6.3 场景模板。|
...
| P3 | 确认送达升级状态机 + 名册视图 + 每周演练 cron | 🟡 **部分** | **已有**：服务端订阅并持久化 ack（§5.3）；`critical`/`emergency` 的 T1/T2/T3 状态机（ntfy 递增优先级重发 + T3 标 `UNREACHABLE`）；`GET /api/alerts/acks`（已确认／在线未确认）与 `GET /api/alerts/escalations`。**名册 UI 已实现**：历史页每条 critical/emergency 可展开出确认名册——**红=在线但未确认**（附行动指引：打电话/去现场）、绿=已确认，并显示当前升级阶段与 UNREACHABLE 判定；界面上保留「确认的是设备、不是人」的注脚。**每周演练 cron 已实现**（§3.4）。**仍缺**：`reissued_at`/`escalation_phase`/`requires_ack` 三个签名字段与 accept-gate「再次拉响」（需 **schema v2**，破坏性变更）→ 客户端目前**不会**因升级重新拉响；`alerts/<id>/escalation/<deviceId>` topic（状态改由 HTTP 暴露）。|
