...
| P3 | 确认送达升级状态机 + 名册视图 + 每周演练 cron | 🟡 **部分** | **已有**：服务端订阅并持久化 ack（§5.3）；`critical`/`emergency` 的 T1/T2/T3 状态机（ntfy 递增优先级重发 + T3 标 `UNREACHABLE`）；`GET /api/alerts/acks`（已确认／在线未确认）与 `GET /api/alerts/escalations`。**仍缺**：`reissued_at`/`escalation_phase`/`requires_ack` 三个签名字段与 accept-gate「再次拉响」（需 **schema v2**，破坏性变更）→ 故客户端目前**不会**因升级重新拉响；`alerts/<id>/escalation/<deviceId>` topic；三色名册 UI；**每周演练 cron（§3.4）仍完全没有**。|
